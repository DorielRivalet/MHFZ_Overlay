// © 2023 The mhfz-overlay developers.
// Use of this source code is governed by a MIT license that can be
// found in the LICENSE file.

namespace MHFZ_Overlay.Models;

using System;

// TODO: ORM
public class PlayerInventory
{
    public DateTime? CreatedAt { get; set; }

    public string? CreatedBy { get; set; } = string.Empty;

    public long? PlayerInventoryID { get; set; }

    public long? RunID { get; set; }

    public long Item1ID { get; set; }

    public long Item1Quantity { get; set; }

    public long Item2ID { get; set; }

    public long Item2Quantity { get; set; }

    public long Item3ID { get; set; }

    public long Item3Quantity { get; set; }

    public long Item4ID { get; set; }

    public long Item4Quantity { get; set; }

    public long Item5ID { get; set; }

    public long Item5Quantity { get; set; }

    public long Item6ID { get; set; }

    public long Item6Quantity { get; set; }

    public long Item7ID { get; set; }

    public long Item7Quantity { get; set; }

    public long Item8ID { get; set; }

    public long Item8Quantity { get; set; }

    public long Item9ID { get; set; }

    public long Item9Quantity { get; set; }

    public long Item10ID { get; set; }

    public long Item10Quantity { get; set; }

    public long Item11ID { get; set; }

    public long Item11Quantity { get; set; }

    public long Item12ID { get; set; }

    public long Item12Quantity { get; set; }
    public long Item13ID { get; set; }

    public long Item13Quantity { get; set; }

    public long Item14ID { get; set; }

    public long Item14Quantity { get; set; }

    public long Item15ID { get; set; }

    public long Item15Quantity { get; set; }

    public long Item16ID { get; set; }

    public long Item16Quantity { get; set; }

    public long Item17ID { get; set; }

    public long Item17Quantity { get; set; }

    public long Item18ID { get; set; }

    public long Item18Quantity { get; set; }

    public long Item19ID { get; set; }

    public long Item19Quantity { get; set; }

    public long Item20ID { get; set; }

    public long Item20Quantity { get; set; }
}
